...
2. 通过 5 套真实 PPTX 样板确认视觉方向；每套样板都有一张由 PPTX 导出的独立 PNG 预览，样板里带标题、正文、指标和图表标签，方便判断字体层级和排版密度；不得使用 SVG 拼凑假图，也不得用整页生图冒充最终可编辑页面；候选页必须是融合式版面，大面积正文容器、大面积图表容器和指标描边框数量必须为 0，文字、数字和图表要嵌入背景留白。
