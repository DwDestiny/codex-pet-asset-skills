---
name: animation-sprite-set
description: Plan, generate, QA, and package continuous raster animation frame sets, GIF previews, and transparent spritesheet atlases. Use when the user asks for a GIF animation, a set of animation frames, sprite rows, web/game animation assets, or Codex pet-like continuous animation materials.
---
...
适合输出：连续帧 PNG、单状态 GIF、透明 spritesheet、Codex pet-like atlas、网页/游戏动画素材。

不适合输出：单张透明素材；这类转给 `$transparent-asset-generation`。
...
5. 如需要，先用 `$transparent-asset-generation` 把每帧处理成透明 PNG。
...
如果是 Codex pet-like 资产，先读 `references/codex-pet-rows.md`，使用官方 hatch-pet 拆出的 8x9、192x208、9 行状态口径。
...
```bash
python "${CODEX_HOME:-$HOME/.codex}/skills/animation-sprite-set/scripts/compose_sprite_set.py" \
