...
商务级风格候选还有一条硬门槛：**禁止大白框贴背景**。正文区、指标区和图表区必须和背景形成同一个视觉系统，可以用开放式版面、纸纹层、玻璃层、光带、细线分隔、风格化坐标区域或小型指标组；不要用两个大面积纯白矩形分别装正文和图表。候选页的大面积正文容器和大面积图表容器数量必须为 0，浅色风格也要通过留白、细线、层次和背景纹理融合，而不是靠白色卡片遮住背景。
...
- 风格候选里的正文和图表不能放进大面积矩形容器；允许小型指标 tile，但大面积正文容器和大面积图表容器必须为 0。
